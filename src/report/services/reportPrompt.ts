...
export function buildFallbackNarrative(payload: C1LitePayload): C1LiteNarrative {
  ...
  const analyst_note = `This report evaluates ${schools.length} institution${schools.length === 1 ? "" : "s"} for ${student.display_name} using only the federally sourced figures in this document; no number here is estimated or computed beyond what the underlying dataset reports. Where a figure is not published, it is shown as "not published" rather than substituted with a typical value. This report does not rank schools or recommend one over another — the following pages lay out cost, admissions, earnings, and debt so your family can weigh the trade-offs directly. Confirm all figures with each institution before making a decision.`;
