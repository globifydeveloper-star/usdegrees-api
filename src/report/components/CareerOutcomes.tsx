...
export default function CareerOutcomes({
  ...
  return (
    <div style={pageStyle}>
      <div>
        <PageHeader sectionLabel="SECTION 03 · EARNINGS & DEBT" title="Program Earnings & Typical Debt" />
        ...
        <div style={{ backgroundColor: theme.color.panelBg, borderRadius: "10px", padding: "14px", border: `1px solid ${theme.color.hairline}`, marginBottom: "20px", fontSize: "11.5px", lineHeight: "1.5" }}>
          ...
          <p style={{ margin: "0 0 12px 0", color: theme.color.ink }}>{narrative.earnings_paragraph}</p>
          <div style={{ fontWeight: 700, color: theme.color.navy, textTransform: "uppercase", fontSize: "10px", marginBottom: "4px" }}>
          ...
          <p style={{ margin: 0, color: theme.color.ink }}>{narrative.debt_burden_paragraph}</p>
        </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "10.5px", marginBottom: "25px", border: `1px solid ${theme.color.hairline}` }}>
          <thead>
            <tr style={{ backgroundColor: theme.color.navy, color: theme.color.white, textAlign: "left" }}>
              ...
              <th style={{ padding: "8px", fontWeight: 600, textAlign: "right" }}>Program Earnings (Yr 10)</th>
              <th style={{ padding: "8px", fontWeight: 600, textAlign: "right" }}>Typical Debt</th>
              <th style={{ padding: "8px", fontWeight: 600, textAlign: "right" }}>Debt-to-Income Label</th>
          ...
          <tbody>
            {schools.map((s, i) => (
              <tr key={i} style={{ borderBottom: `1px solid ${theme.color.hairline}`, backgroundColor: i % 2 === 0 ? theme.color.panelBgAlt : theme.color.panelBg }}>
                <td style={{ padding: "8px", fontWeight: 700, color: theme.color.navy }}>{s.name}</td>
                <td style={{ padding: "8px", textAlign: "right", color: theme.color.navy, fontWeight: 700 }}>
                  {s.program_earnings != null ? `$${s.program_earnings.toLocaleString()}` : "Not published"}
                </td>
                <td style={{ padding: "8px", textAlign: "right", color: theme.color.ink }}>
                  {s.avg_debt != null ? `$${s.avg_debt.toLocaleString()}` : "Not published"}
                </td>
                <td style={{ padding: "8px", textAlign: "right", color: theme.color.ink, fontWeight: 600 }}>
                  {s.debt_ratio_text ?? "Not published"}
                </td>
                <td style={{ padding: "8px", textAlign: "right", color: theme.color.ink, fontWeight: 700 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
          {schools.map((s, i) => (
            <div key={i} style={{ fontSize: "9px", color: theme.color.inkFaint }}>
              {s.name}: earnings {s.earnings_vintage ?? "n/a"}{s.earnings_method_flag ? ` (${s.earnings_method_flag})` : ""}, debt {s.debt_vintage ?? "n/a"}
            </div>
          ))}
        </div>
      </div>
